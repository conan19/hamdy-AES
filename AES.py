...
def plaintext():
	try:
		...
		if(real_size % block_size != 0):
			new_string= i + (16-real_size % 16)*("x")
		else:
			new_string=i
...
def decryption():
 try:
	cipher=input("list of 16 hex to decrypt: ")
	cipher_state=numpy.array(cipher).reshape(4,4).transpose()
...
def mainFun():
 try:
	...
	if i == 2:
		output=decryption()
		text=enhance(output)
		print "text is: " ,''.join(text)
	if i==1:
		output=encryption()
		print "the cipher text is: ", output
	else:
